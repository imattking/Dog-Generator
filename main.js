...
// set up fetch for image from Dog CEO API    

const dogImg = document.querySelector('.dog-img');

function getDogImage(url) {
    fetch(url) // grabbing the API url from above
        .then( res => res.json() ) // get response as .json object
        .then( data => {
            dogImg.src = data.message;
            console.log(data.message); // extract / parse out 'message' from .json and attach to img src
        })
        .catch( err => {
            console.log( `err: ${err} `);
        })
...
// set up dog info to populate once breed is selected 
...
const doggoInfo = {
    ...
    assignLikes() {
        ...
        document.querySelector('#likes').innerHTML = `Likes: ${this.likes[0]}, ${this.likes[1]}`
    },

    assignDislikes() {
        this.dislikes = this.fisherYatesShuffle(this.dislikesList).slice(0, 2);
        document.querySelector('#dislikes').innerHTML = `Dislikes: ${this.dislikes[0]}, ${this.dislikes[1]}`
    },

    assignFact() {
        this.fact = this.factList[ Math.floor(Math.random() * this.factList.length) ]
        document.querySelector('#fun-fact').innerHTML = `Additional Info: ${this.fact}`;
    },
